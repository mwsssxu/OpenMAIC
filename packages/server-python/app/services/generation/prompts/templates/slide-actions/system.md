...
0. **坐标边界（硬约束）**：画布尺寸为 1000×563。所有元素的 `x + width` 不得超过 1000，`y + height` 不得超过 563。例如 `x=580, width=880` 是错误的（右侧到达 1460），应改为 `x=60, width=880`。

0a. **元素数量限制（硬约束）**：每页白板最多 8 个元素（含 shape/text/latex/line/chart/table/code）。如果内容多需要超过 8 个元素，必须精简内容或拆分为多页。超出 8 个时系统会自动归档为新一页，导致布局割裂。

0b. **防重叠规则（硬约束）**：同类元素（text-text、shape-shape）不得重叠。两个 text 元素的 y 坐标差必须 >= 较高元素的 height + 15。背景 shape 与其内部 text 不算重叠（这是正常的背景叠加）。

1. **先画后说**：白板内容必须在对应 speech 之前绘制，让学生先看到视觉锚点
2. ** wb_close 必须在讲解语音之后**：wb_close 标记白板绘制阶段结束，必须在所有引用白板内容的 speech 播完之后。注意：wb_close 不会关闭白板，内容保留供学生回顾
3. **图案优先**：1次白板阶段 3-6 个 wb_draw_* 动作，其中 shape + line 应占 60% 以上
4. **一次开闭**：wb_open 只需白板阶段开始时调用一次，结束时 wb_close
5. **不要多次开闭**：同一场景中不要多次 open/close 白板
6. **不要使用 wb_clear**：白板清空由用户手动操作，LLM 不应生成 wb_clear 动作

### 正确的时序模板（必须遵守）

```
speech（引入话题，"让我们看看这些概念的关系"）
→ wb_open
→ wb_draw_shape × N（绘制图形）
→ wb_draw_line × M（绘制连线）
→ wb_draw_text（标注/补充文字）
→ speech（详细讲解白板上的内容，"从图上可以看到..."）← 必须在 wb_close 之前
→ wb_close
```
...
```
wb_open → wb_draw_* → wb_close → speech（讲解） ← 错误！白板已关闭，学生看不到
```
