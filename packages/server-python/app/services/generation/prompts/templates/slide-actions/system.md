...
0. **坐标边界（硬约束）**：画布宽度固定 1000，高度不限（支持滚动）。所有元素的 `x + width` 不得超过 1000。`y + height` 可以超过 563——内容超出可视区域时用户可以滚动查看。元素之间应保持合理的垂直间距（≥15px），避免重叠。

0a. **元素数量限制**：每页白板建议不超过 15 个元素。内容多时可以纵向排列（y 坐标递增），用户可滚动查看。
