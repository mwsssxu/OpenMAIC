...
1. **场景内容渲染**：根据场景类型展示不同内容——
   ...
   - **交互场景**：HTML交互组件，支持仿真模拟（simulation）、游戏（game）、图表（diagram）、代码编辑（code）、3D可视化（visualization3d）等Widget类型，AI教师可通过widget_highlight、widget_setState等动作操控Widget内容进行教学引导
...
1. **邀请码展示**：大号展示用户唯一6位字母数字组合邀请码，"分享邀请码"按钮调用系统分享对话框。
...
1. **Token 套餐选择**：可点击的套餐卡片列表，展示名称、Token数量、价格（元）、额外赠送Token和描述。包括体验包（50 Token，¥6）、学习包（200 Token含20赠送，¥18）、畅学包（600 Token含100赠送，¥48）。另有Pro订阅套餐：Pro月卡（¥19/月，含50 Token）、Pro年卡（¥190/年）。
...
应用底部 Tab 栏包含4个主入口和1个居中创建按钮：

| Tab | 图标 | 功能 |
|-----|------|------|
| 首页 | home | 学习仪表盘，功能快捷入口 |
| 课程 | book | 课程列表与管理 |
| 创建 | add-circle | 创建新课程（居中突出按钮） |
...
1. **场景内容区**：占据主要屏幕空间，根据场景类型动态渲染：
   ...
   - **交互场景**：HTML交互组件（仿真模拟、游戏、图表、代码编辑器、3D可视化等Widget）
...
![问答悬赏](./screenshots/07-questions.png)
...
3. **套餐选择区**：垂直排列的可点击套餐卡片，每张包含：
   - 套餐名称（"体验包"、"学习包"、"畅学包"）
   - Token数量（"50 Token"、"200 Token（含20赠送）"、"600 Token（含100赠送）"）
   - 价格（以人民币显示，"¥6"、"¥18"、"¥48"）
   - 套餐描述（如"适合新手尝试"、"最受欢迎选择"）
   - 当前选中状态（高亮边框或颜色区分）
   另有Pro订阅套餐区域：Pro月卡（¥19/月，含50 Token）、Pro年卡（¥190/年）
...
![支付充值](./screenshots/09-payment.png)
![支付充值](./screenshots/07-payment.png)
![支付充值](./screenshots/08-payment.png)
...
![知识管理](./screenshots/12-knowledge.png)
![知识管理](./screenshots/13-knowledge.png)
...
![游戏化激励](./screenshots/10-payment.png)
...
![邀请系统](./screenshots/14-invite.png)
#### 5.3.12 钱包
...
![笔记管理](./screenshots/10-notes.png)
![笔记管理](./screenshots/11-notes.png)
...
![学习搭子](./screenshots/01-classmate.png)
![学习搭子](./screenshots/02-classmate.png)
...
4. **偏好设置模态框**：输入目标标签、选择进度等级（初级/中级/高级）、保存偏好。
![学习搭子](./screenshots/03-classmate.png)
